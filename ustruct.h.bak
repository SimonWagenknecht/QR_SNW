...
// Weichenkreis
typedef struct weis {
	int		 ipa_Hoch_QRG_aus;			// Parameter in [K] / 10, der den Betriebszustand Hochtemperaturquellenregeneration deaktiviert
	int		 ipa_Hoch_QRG_ein;			// Parameter in [K] / 10, der den Betriebszustand Hochtemperaturquellenregeneration aktiviert
	int		 ipa_Nieder_QRG_ein;		// Parameter in [K] / 10, der den Betriebszustand Niedertemperaturquellenregeneration aktiviert
	int		 ipa_eTankgrenze;				// Parameter in [°C] / 10, Grenzwert des Etanks
	int	 	 ipa_eTank_Minlaufzeit;	// Parameter in [min], erst nach Ablauf des Counters, wird die die Grenztemperatur des Etanks kontrolliert
	int		 ipa_Funktion_TP;				// Parameter, der die Funktionen des Taupunkts aktiviert oder deaktiviert
	int		 ipa_Taupunktgrenze;		// Parameter in [K] / 10, dT-Grenzwert zum Taupunkt
...
typedef struct weid {
	...
	char	chbz_eTankschutz;				// Betriebszustand eTankschutz = eTank voll	
	char	chbz_Taupunktschutz;		// Betriebszustand Taupunktschutz
	char	chbz_Nieder_QRG;				// Betriebszustand Niedertemperaturquellenregeneration
	char	chbz_Hoch_QRG;					// Betriebszustand Hochtemperaturquellenregeneration
	char	chbz_Quellenanhebung;		// Betriebszustand Quellenanhebung
	 int	 iCo_QRG;								// beginnt von ipa_eTank_Minlaufzeit abwärts zu zählen nach Aktivierung eines Betriebszustands bzgl Quellenregeneration 
	 int	TQRL_recent;						// im RAM gespeicherter Wert, nach dem die Quellenpumpe ausgeschalten wurde
...
// Quellenkreis
typedef struct qs {
	int		 ipa_Sollwert_WPU_VL;		// Vorlauftemperatur im Wärmepumpenbetrieb
	char	chpa_pu_Hau;						// Handsteuerung der Weichenpumpe PU WEI aktivieren
	char	chpa_pu_ein;						// Weichenpumpe PU WEI im Handbetrieb ein-/ausschalten
	char	chpa_rv_Hau;						// Handsteuerung des Weichenventils RV WEI aktivieren
	int		 ipa_rv_stellung;				// Ventilstellung von RV WEI im Handbetrieb
...
typedef struct qd {
	...
	char	chbz_WPU_Versorgung;		// Betrtiebszustand Wärmepumpenversorgung	
